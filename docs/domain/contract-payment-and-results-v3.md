...
Antes de configurar un reset, los pagos históricos ya afectan la proyección: el estimador reconstruye su saldo y omite las cuotas ya cubiertas. Por ello un calendario importado que termina el `2026-07-15` empieza a proyectarse en la siguiente cuota contractual, no desde la fecha de inicio del préstamo. Las comparaciones de escenarios usan ese mismo saldo, calendario de cuotas y fase de tasa; con reset usan el saldo reportado y la fecha final bancaria. Los escenarios no vuelven al monto original.
...
4. El gráfico tiene eje X temporal, eje Y monetario, cuadrícula, etiquetas compactas, nombre y descripción accesibles, controles de rango y detalle al enfocar o pasar el cursor por un punto. Abre en el plazo total y el rango se acota con fecha inicial/final o se restablece al total. Permite seleccionar saldo, cuota total, interés, aporte total a principal y aporte extraordinario a principal. Cada señal activa se dibuja para la base y los escenarios elegidos con un color propio; la base usa mayor intensidad y línea continua, y cada escenario un patrón de punteado propio. La leyenda separa ambos códigos. El detalle es una tabla con señales en filas y fuentes en columnas; un clic, Enter o Espacio fija/libera el periodo inspeccionado. La tabla de amortización conserva cabecera, contraste, columnas numéricas alineadas, navegación de páginas y orden alternable por fecha.
