...
4. El gráfico tiene eje X temporal, eje Y monetario, cuadrícula, etiquetas compactas, nombre y descripción accesibles, controles de rango y detalle al enfocar o pasar el cursor por un punto. Permite seleccionar saldo, cuota total, interés, principal y extraordinario. Cada señal activa se dibuja para la base y los escenarios elegidos con color y patrón constantes; la intensidad diferencia la fuente. El detalle agrupa las señales activas por fuente y un clic, Enter o Espacio fija/libera el periodo inspeccionado. La tabla conserva cabecera, contraste, columnas numéricas alineadas, navegación de páginas y orden alternable por fecha.
